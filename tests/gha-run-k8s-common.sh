...
function helm_helper() {
	...
	local helm_extra_cmdline_array
	local max_tries
	local interval
	local i

	helm_extra_cmdline_array=()
	if [[ -z "${HELM_IMAGE_REFERENCE}" ]]; then
		die "HELM_IMAGE_REFERENCE environment variable cannot be empty."
	fi
	helm_extra_cmdline_array+=("--set image.reference=\"${HELM_IMAGE_REFERENCE}\"")

	if [[ -z "${HELM_IMAGE_TAG}" ]]; then
		die "HELM_IMAGE_TAG environment variable cannot be empty."
	fi
	helm_extra_cmdline_array+=("--set image.tag=\"${HELM_IMAGE_TAG}\"")

	if [[ "${HELM_DEFAULT_INSTALLATION}" = "false" ]]; then
		[[ -n "${HELM_DEBUG}" ]] && helm_extra_cmdline_array+=("--set env.debug=\"${HELM_DEBUG}\"")
		[[ -n "${HELM_SHIMS}" ]] && helm_extra_cmdline_array+=("--set env.shims=\"${HELM_SHIMS}\"")
		[[ -n "${HELM_DEFAULT_SHIM}" ]] && helm_extra_cmdline_array+=("--set env.defaultShim=\"${HELM_DEFAULT_SHIM}\"")
		[[ -n "${HELM_CREATE_RUNTIME_CLASSES}" ]] && helm_extra_cmdline_array+=("--set env.createRuntimeClasses=\"${HELM_CREATE_RUNTIME_CLASSES}\"")
		[[ -n "${HELM_CREATE_DEFAULT_RUNTIME_CLASS}" ]] && helm_extra_cmdline_array+=("--set env.createDefaultRuntimeClass=\"${HELM_CREATE_DEFAULT_RUNTIME_CLASS}\"")
		[[ -n "${HELM_ALLOWED_HYPERVISOR_ANNOTATIONS}" ]] && helm_extra_cmdline_array+=("--set env.allowedHypervisorAnnotations=\"${HELM_ALLOWED_HYPERVISOR_ANNOTATIONS}\"")
		[[ -n "${HELM_SNAPSHOTTER_HANDLER_MAPPING}" ]] && helm_extra_cmdline_array+=("--set env.snapshotterHandlerMapping=\"${HELM_SNAPSHOTTER_HANDLER_MAPPING}\"")
		[[ -n "${HELM_AGENT_HTTPS_PROXY}" ]] && helm_extra_cmdline_array+=("--set env.agentHttpsProxy=\"${HELM_AGENT_HTTPS_PROXY}\"")
		[[ -n "${HELM_AGENT_NO_PROXY}" ]] && helm_extra_cmdline_array+=("--set env.agentNoProxy=\"${HELM_AGENT_NO_PROXY}\"")
		[[ -n "${HELM_PULL_TYPE_MAPPING}" ]] && helm_extra_cmdline_array+=("--set env.pullTypeMapping=\"${HELM_PULL_TYPE_MAPPING}\"")
		[[ -n "${HELM_HOST_OS}" ]] && helm_extra_cmdline_array+=("--set env.hostOS=\"${HELM_HOST_OS}\"")
	fi
	helm_extra_cmdline="${helm_extra_cmdline_array[*]}"
	...
	while true; do
		helm_cmd="helm upgrade --namespace=kube-system --debug --install kata-deploy "${helm_chart_dir}" "${helm_extra_cmdline}""
		echo "${helm_cmd}"
		ret=$("${helm_cmd}")
		if [[ ${ret} -eq 0 ]]; then
